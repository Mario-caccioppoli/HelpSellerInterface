...
export class testRegex {
  ...
  regexEmail(email: string) {
    ...
    return regEx.test(email);
      ...
      regexPrezzoVolumePeso(pvp: string) {
        const regEx = /^\d{1,7}(\.\d{2,2})?$/;

        return regEx.test(pvp);
      }

      regexQuantita(quantita: string) {
        const regEx = /^[0-9]+$/;

        return regEx.test(quantita);
      }
